...
public class GameLauncher extends JFrame
{
    ...
    public GameLauncher()
    {
        ...
        mainPanel.add(inventoryPanel, "BAG");
    ...
    public void startGame(boolean isNewGame)
    {
        if (isNewGame) {
            if (bag.isEmpty()) {
                bag.add("Potion"); bag.add("Potion");
                bag.add("Poké Ball"); bag.add("Poké Ball");
                tms.add("TM01 - Tackle");
            }
    ...
    private void setupMenuBar()
    {
        ...
        loadItem.addActionListener(e -> {
            if (loadGame()) {
                startGame(false);
                if (worldPanel != null) worldPanel.refreshMap();
            }
        });
    ...
    public boolean loadGame()
    {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("save.dat"))) {
            ...
            party            = state.party;
            bag              = state.bag;
            if (state.tms != null) tms = state.tms;
            money            = state.money;
            ...
            JOptionPane.showMessageDialog(this, "Game Loaded! Welcome back, " + playerName + "!");
            return true;
        } catch (Exception e) {
    ...
    // -----------------------------------------------------------------------
    // Battle flow — stop/resume movement so the player freezes on encounter
    // -----------------------------------------------------------------------
    public void startBattle(Pokemon enemy, boolean isTrainer, String trainerName, String id)
    {
        this.currentEnemyID = id;

        // FIX: freeze the player before switching to the battle screen.
        // This snaps the player to their current tile, stops the move timer,
        // and clears any held keys so they can't "walk into" the battle.
        if (worldPanel != null) worldPanel.stopMovement();
    ...
    public void endBattle(boolean won)
    {
        ...
        // FIX: restart the movement timer and wipe stale key state so the
        // player doesn't immediately sprint in whatever direction was held
        // during the battle screen.
        if (worldPanel != null) worldPanel.resumeMovement();
    ...
    public void closeInventory()
    {
        cardLayout.show(mainPanel, "WORLD");
        if (worldPanel != null) {
            worldPanel.resumeMovement();   // also resume here in case timer was paused
            worldPanel.requestFocusInWindow();
        }
    }
...
class InventoryPanel extends JPanel
{
    ...
    public void refresh()
    {
        ...
        if ("Items".equals(cat)) {
            for (String item : GameLauncher.bag)  listModel.addElement(item);
        } else {
            for (String tm   : GameLauncher.tms)  listModel.addElement(tm);
        }
    }
    ...
    private void useSelectedItem()
    {
        ...
        if (selected == null) { JOptionPane.showMessageDialog(this, "Please select an item first!"); return; }
        if (GameLauncher.party.isEmpty()) { JOptionPane.showMessageDialog(this, "You have no Pokémon to use this on!"); return; }
        ...
        String target = (String) JOptionPane.showInputDialog(this,
            "Use " + selected + " on which Pokémon?", "Select Target",
            JOptionPane.QUESTION_MESSAGE, null, partyNames, partyNames[0]);

        if (target == null) return;

        int targetIndex = -1;
        for (int i = 0; i < partyNames.length; i++) if (partyNames[i].equals(target)) targetIndex = i;
        if (targetIndex < 0) return;

        Pokemon p = GameLauncher.party.get(targetIndex);

        if (selected.equals("Potion")) {
            if (p.currentHp == p.maxHp) { JOptionPane.showMessageDialog(this, p.name + "'s HP is already full!"); return; }
            ...
            JOptionPane.showMessageDialog(this, "TM logic goes here!");
        }
    }
}
...
class MainMenuPanel extends JPanel
{
    public MainMenuPanel(GameLauncher game)
    {
        ...
        newGameBtn.addActionListener(e -> {
            ...
            if (!typed.isEmpty()) GameLauncher.playerName = typed;
            game.startGame(true);
        });
        add(newGameBtn, gbc);
        ...
        loadGameBtn.addActionListener(e -> { if (game.loadGame()) game.startGame(false); });
        add(loadGameBtn, gbc);
    }
}
...
class Pokemon implements Serializable
{
    ...
    public Pokemon(String name, String type, int level)
    {
        ...
        if (moves.isEmpty()) learnBaseMoves();
    }
    ...
    private void learnBaseMoves()
    {
        ...
        switch (type) {
            case "FIRE":     moves.add(new Move("Ember","FIRE",40,25));
                             if(level>=12) moves.add(new Move("Flamethrower","FIRE",90,15)); break;
            case "WATER":    moves.add(new Move("Water Gun","WATER",40,25));
                             if(level>=12) moves.add(new Move("Bubblebeam","WATER",65,20)); break;
            case "GRASS":    moves.add(new Move("Vine Whip","GRASS",45,25));
                             if(level>=12) moves.add(new Move("Razor Leaf","GRASS",55,25)); break;
            case "ELECTRIC": moves.add(new Move("Thundershock","ELECTRIC",40,30));
                             if(level>=12) moves.add(new Move("Thunderbolt","ELECTRIC",90,15)); break;
            case "ROCK":
            case "GROUND":   moves.add(new Move("Rock Throw","ROCK",50,15));
                             if(level>=12) moves.add(new Move("Earthquake","GROUND",100,10)); break;
            case "PSYCHIC":  moves.add(new Move("Confusion","PSYCHIC",50,25));
                             if(level>=12) moves.add(new Move("Psychic","PSYCHIC",90,10)); break;
            case "BUG":      moves.add(new Move("String Shot","BUG",10,40));
                             if(level>=10) moves.add(new Move("Twin Needle","BUG",25,20)); break;
            case "GHOST":    moves.add(new Move("Lick","GHOST",30,30));
                             if(level>=12) moves.add(new Move("Shadow Ball","GHOST",80,15)); break;
            case "FIGHTING": moves.add(new Move("Low Kick","FIGHTING",40,20));
                             if(level>=12) moves.add(new Move("Karate Chop","FIGHTING",50,25)); break;
            case "POISON":   moves.add(new Move("Poison Sting","POISON",15,35));
                             if(level>=12) moves.add(new Move("Sludge","POISON",65,20)); break;
            case "ICE":      moves.add(new Move("Ice Shard","ICE",40,30));
                             if(level>=12) moves.add(new Move("Ice Beam","ICE",90,10)); break;
            case "DRAGON":   moves.add(new Move("Twister","DRAGON",40,20));
                             if(level>=12) moves.add(new Move("Dragon Claw","DRAGON",80,15)); break;
            default:         if(level>=10) moves.add(new Move("Quick Attack","NORMAL",40,30)); break;
        }
    }
    ...
    { currentHp=maxHp; for(Move m:moves) m.currentAp=m.maxAp; }

    public void takeDamage(int amt)
    { currentHp-=amt; if(currentHp<0) currentHp=0; }
