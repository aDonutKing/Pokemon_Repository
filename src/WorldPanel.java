package PART2;
import javax.swing.*;
import java.awt.*;
import java.awt.event.*;
import java.util.ArrayList;
import java.util.List;

public class WorldPanel extends JPanel
{
    private GameLauncher game;
    private final int TILE_SIZE = 48;

    private boolean showPokedex = false;
    private boolean canSurf = false;

    // -----------------------------------------------------------------------
    // Map Constants — ordered as they appear in Pokémon Red
    // -----------------------------------------------------------------------
    private final int MAP_PALLET      = 0;
    private final int MAP_ROUTE1      = 1;   // Pallet → Viridian
    private final int MAP_VIRIDIAN    = 2;
    private final int MAP_ROUTE2      = 3;   // Viridian → Pewter (via Forest)
    private final int MAP_FOREST      = 4;   // Viridian Forest
    private final int MAP_PEWTER      = 5;
    private final int MAP_ROUTE3      = 6;   // Pewter → Mt. Moon
    private final int MAP_MT_MOON     = 7;
    private final int MAP_ROUTE4      = 8;   // Mt. Moon → Cerulean
    private final int MAP_CERULEAN    = 9;
    private final int MAP_ROUTE24     = 10;  // Cerulean → Bill's (north) — replaces Route 5 as first south exit
    private final int MAP_ROUTE5      = 11;  // Cerulean → Saffron (south)
    private final int MAP_SAFFRON     = 14;
    private final int MAP_ROUTE6      = 15;  // Saffron → Vermilion
    private final int MAP_VERMILION   = 16;
    private final int MAP_ROUTE11     = 17;  // Vermilion → Lavender (east)
    private final int MAP_ROUTE9      = 18;  // Cerulean → Lavender (east short-cut via Rock Tunnel)
    private final int MAP_ROUTE10     = 19;  // Rock Tunnel area
    private final int MAP_ROUTE7      = 20;  // Celadon → Saffron
    private final int MAP_CELADON     = 21;
    private final int MAP_ROUTE8      = 22;  // Lavender → Saffron (west)
    private final int MAP_LAVENDER    = 23;
    private final int MAP_ROUTE12     = 24;  // Lavender → Fuchsia (south)
    private final int MAP_ROUTE16     = 25;  // Celadon → Fuchsia (west gate)
    private final int MAP_ROUTE15     = 26;  // Fuchsia ← Route 12 loop
    private final int MAP_FUCHSIA     = 27;
    private final int MAP_ROUTE13     = 28;  // Pallet-side connector (Route 13)
    private final int MAP_ROUTE14     = 29;  // Fuchsia connector
    private final int MAP_ROUTE19     = 30;  // Fuchsia → Seafoam (south sea)
    private final int MAP_ROUTE20     = 31;  // Seafoam → Cinnabar (sea)
    private final int MAP_CINNABAR    = 32;
    private final int MAP_ROUTE21     = 33;  // Cinnabar → Pallet (sea)
    private final int MAP_GYM_PEWTER  = 34;
    private final int MAP_GYM_CERULEAN= 35;
    private final int MAP_GYM_VERMILION=36;
    private final int MAP_GYM_CELADON = 37;
    private final int MAP_GYM_FUCHSIA = 38;
    private final int MAP_GYM_CINNABAR= 39;
    private final int MAP_CENTER      = 40;
    private final int MAP_MART        = 41;
    private final int MAP_WARDEN_HOUSE= 42;

    private static final int TOTAL_MAPS = 43;

    private int currentMapId = MAP_PALLET;
    private int returnMapId  = MAP_VIRIDIAN;
    private int[][] currentMap;
    private List<NPC>      npcs   = new ArrayList<>();
    private List<MapLabel> labels = new ArrayList<>();

    private int[][][] allMaps = new int[TOTAL_MAPS][][];
    private List<List<NPC>>      allNPCs   = new ArrayList<>();
    private List<List<MapLabel>> allLabels = new ArrayList<>();

    // -----------------------------------------------------------------------
    // Smooth movement state
    // -----------------------------------------------------------------------
    private int playerX = 10, playerY = 10;   // tile position

    // Sub-tile pixel offset (for smooth scrolling)
    private float offsetX = 0f, offsetY = 0f;

    // Queued movement direction
    private int pendingDX = 0, pendingDY = 0;

    // Is a step animation currently playing?
    private boolean isMoving = false;

    // Speed: pixels moved per timer tick. 48px tile / 6px per tick = 8 ticks per step = ~133ms
    private static final float MOVE_SPEED = 6f;

    // The tile we are animating toward (set when step begins)
    private int targetX, targetY;

    private Timer moveTimer;

    // Key-held tracking for held-key auto-repeat
    private final boolean[] keysHeld = new boolean[256];

    public WorldPanel(GameLauncher game)
    {
        this.game = game;

        for (int i = 0; i < TOTAL_MAPS; i++) {
            allNPCs.add(new ArrayList<>());
            allLabels.add(new ArrayList<>());
        }

        setPreferredSize(new Dimension(800, 600));
        setBackground(Color.BLACK);
        setFocusable(true);
        loadMap(MAP_PALLET, 10, 10);

        // Movement timer — ticks at ~60fps
        moveTimer = new Timer(16, e -> tickMovement());
        moveTimer.start();

        addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                int code = e.getKeyCode();

                if (showPokedex) {
                    if (code == KeyEvent.VK_P || code == KeyEvent.VK_ESCAPE) {
                        showPokedex = false;
                        repaint();
                    }
                    return;
                }

                if (code == KeyEvent.VK_P) {
                    showPokedex = true;
                    repaint();
                    return;
                }

                if (code == KeyEvent.VK_B) {
                    game.openInventory();
                    return;
                }

                if (code == KeyEvent.VK_SPACE) {
                    interact();
                    return;
                }

                if (code < keysHeld.length) keysHeld[code] = true;
            }

            @Override
            public void keyReleased(KeyEvent e) {
                int code = e.getKeyCode();
                if (code < keysHeld.length) keysHeld[code] = false;
            }
        });
    }

    // -----------------------------------------------------------------------
    // Movement timer — called every ~16ms
    // -----------------------------------------------------------------------
    private void tickMovement()
    {
        if (isMoving) {
            // Continue animating toward target tile
            float remainX = (targetX - playerX) * TILE_SIZE - offsetX;
            float remainY = (targetY - playerY) * TILE_SIZE - offsetY;

            float distLeft = (float) Math.sqrt(remainX * remainX + remainY * remainY);

            if (distLeft <= MOVE_SPEED) {
                // Snap to target tile — step complete
                playerX  = targetX;
                playerY  = targetY;
                offsetX  = 0f;
                offsetY  = 0f;
                isMoving = false;

                onStepComplete();

            } else {
                offsetX += (remainX / distLeft) * MOVE_SPEED;
                offsetY += (remainY / distLeft) * MOVE_SPEED;
            }

            repaint();

        } else {
            // Read currently held keys to start a new step
            int dx = 0, dy = 0;
            if (keysHeld[KeyEvent.VK_W] || keysHeld[KeyEvent.VK_UP])    dy = -1;
            else if (keysHeld[KeyEvent.VK_S] || keysHeld[KeyEvent.VK_DOWN])  dy =  1;
            else if (keysHeld[KeyEvent.VK_A] || keysHeld[KeyEvent.VK_LEFT])  dx = -1;
            else if (keysHeld[KeyEvent.VK_D] || keysHeld[KeyEvent.VK_RIGHT]) dx =  1;

            if (dx != 0 || dy != 0) tryStep(dx, dy);
        }
    }

    // -----------------------------------------------------------------------
    // Attempt to move one tile in direction (dx, dy)
    // -----------------------------------------------------------------------
    private void tryStep(int dx, int dy)
    {
        int nx = playerX + dx;
        int ny = playerY + dy;

        // --- Map transition checks (same logic as before) ---
        if (handleMapTransition(nx, ny)) return;

        // Bounds
        if (nx < 0 || ny < 0 || nx >= currentMap.length || ny >= currentMap[0].length) return;

        // Water check
        if (currentMap[nx][ny] == 5 && !canSurf) return;

        // Collision
        int tile = currentMap[nx][ny];
        if (tile == 1 || tile == 2 || tile == 6) return;

        // NPC collision
        for (NPC n : npcs) {
            if (n.x == nx && n.y == ny) {
                if (n.type.equals("TRAINER") || n.type.equals("LEADER")) {
                    game.startBattle(n.party, true, n.name, n.id);
                } else if (n.type.equals("HEALER")) {
                    for (Pokemon p : GameLauncher.party) p.healFull();
                    JOptionPane.showMessageDialog(this, "Nurse Joy: Your Pokémon are fully healed!");
                }
                return;
            }
        }

        // Door tiles — instant warp (no animation)
        if (tile >= 98 && tile <= 215) {
            handleDoorTile(tile);
            return;
        }

        // Start smooth step animation
        targetX  = nx;
        targetY  = ny;
        isMoving = true;
    }

    // -----------------------------------------------------------------------
    // Called when the player's foot hits a new tile
    // -----------------------------------------------------------------------
    private void onStepComplete()
    {
        int tile = currentMap[playerX][playerY];

        // Tall grass
        if (tile == 4 && Math.random() < 0.15) {
            game.startBattle(Pokemon.generateWild(4, 12), false, "Wild Pokemon", "WILD");
        }
        // Water
        else if (tile == 5 && Math.random() < 0.10) {
            game.startBattle(new Pokemon("Tentacool", "WATER", 20), false, "Wild Pokemon", "WILD");
        }
    }

    // -----------------------------------------------------------------------
    // Map transitions (geographic edges)
    // -----------------------------------------------------------------------
    private boolean handleMapTransition(int nx, int ny)
    {
        // Pallet
        if (currentMapId == MAP_PALLET    && ny <  0)  { loadMap(MAP_ROUTE1,   6,  28); return true; }
        if (currentMapId == MAP_PALLET    && ny >= 15)  { loadMap(MAP_ROUTE21,  9,   1); return true; }
        // Route 1
        if (currentMapId == MAP_ROUTE1    && ny >= 30)  { loadMap(MAP_PALLET,   9,   1); return true; }
        if (currentMapId == MAP_ROUTE1    && ny <  0)   { loadMap(MAP_VIRIDIAN, 9,  18); return true; }
        // Viridian
        if (currentMapId == MAP_VIRIDIAN  && ny >= 20)  { loadMap(MAP_ROUTE1,   6,   1); return true; }
        if (currentMapId == MAP_VIRIDIAN  && ny <  0)   { loadMap(MAP_ROUTE2,   7,  28); return true; }
        // Route 2
        if (currentMapId == MAP_ROUTE2    && ny >= 30)  { loadMap(MAP_VIRIDIAN, 9,   1); return true; }
        if (currentMapId == MAP_ROUTE2    && ny <  0)   { loadMap(MAP_PEWTER,   9,  18); return true; }
        // Pewter
        if (currentMapId == MAP_PEWTER    && ny >= 20)  { loadMap(MAP_ROUTE2,   7,   1); return true; }
        if (currentMapId == MAP_PEWTER    && nx >= 20)  { loadMap(MAP_ROUTE3,   1,   8); return true; }
        // Route 3
        if (currentMapId == MAP_ROUTE3    && nx <  0)   { loadMap(MAP_PEWTER,  18,  10); return true; }
        if (currentMapId == MAP_ROUTE3    && nx >= 40)  { loadMap(MAP_MT_MOON,  2,  15); return true; }
        // Mt. Moon
        if (currentMapId == MAP_MT_MOON   && nx >= 30)  { loadMap(MAP_ROUTE4,   2,   8); return true; }
        // Route 4
        if (currentMapId == MAP_ROUTE4    && nx <  0)   { loadMap(MAP_MT_MOON, 28,  14); return true; }
        if (currentMapId == MAP_ROUTE4    && nx >= 30)  { loadMap(MAP_CERULEAN, 1,  10); return true; }
        // Cerulean
        if (currentMapId == MAP_CERULEAN  && nx <  0)   { loadMap(MAP_ROUTE4,  28,   8); return true; }
        if (currentMapId == MAP_CERULEAN  && ny <  0)   { loadMap(MAP_ROUTE24,  9,  18); return true; } // North → Route 24
        if (currentMapId == MAP_CERULEAN  && ny >= 20)  { loadMap(MAP_ROUTE5,   9,   1); return true; } // South → Route 5
        // Route 24 (north of Cerulean)
        if (currentMapId == MAP_ROUTE24   && ny >= 20)  { loadMap(MAP_CERULEAN, 10,  1); return true; }
        // Route 5
        if (currentMapId == MAP_ROUTE5    && ny <  0)   { loadMap(MAP_CERULEAN,10,  18); return true; }
        if (currentMapId == MAP_ROUTE5    && ny >= 30)  { loadMap(MAP_SAFFRON, 10,   1); return true; }
        // Saffron
        if (currentMapId == MAP_SAFFRON   && ny <  0)   { loadMap(MAP_ROUTE5,   9,  28); return true; }
        if (currentMapId == MAP_SAFFRON   && ny >= 25)  { loadMap(MAP_ROUTE6,   9,   1); return true; }
        if (currentMapId == MAP_SAFFRON   && nx <  0)   { loadMap(MAP_ROUTE7,  18,   7); return true; }
        if (currentMapId == MAP_SAFFRON   && nx >= 25)  { loadMap(MAP_ROUTE8,   1,   7); return true; }
        // Route 6
        if (currentMapId == MAP_ROUTE6    && ny <  0)   { loadMap(MAP_SAFFRON, 10,  23); return true; }
        if (currentMapId == MAP_ROUTE6    && ny >= 20)  { loadMap(MAP_VERMILION,9,   1); return true; }
        // Vermilion
        if (currentMapId == MAP_VERMILION && ny <  0)   { loadMap(MAP_ROUTE6,   9,  18); return true; }
        if (currentMapId == MAP_VERMILION && nx >= 25)  { loadMap(MAP_ROUTE11,  1,   7); return true; }
        // Route 11
        if (currentMapId == MAP_ROUTE11   && nx <  0)   { loadMap(MAP_VERMILION,23, 10); return true; }
        if (currentMapId == MAP_ROUTE11   && nx >= 40)  { loadMap(MAP_ROUTE9,   1,   7); return true; } // → Route 9/Lavender area
        // Route 9
        if (currentMapId == MAP_ROUTE9    && nx <  0)   { loadMap(MAP_ROUTE11, 38,   7); return true; }
        if (currentMapId == MAP_ROUTE9    && nx >= 40)  { loadMap(MAP_LAVENDER, 1,  10); return true; }
        // Route 7
        if (currentMapId == MAP_ROUTE7    && nx >= 20)  { loadMap(MAP_SAFFRON,  1,  12); return true; }
        if (currentMapId == MAP_ROUTE7    && nx <  0)   { loadMap(MAP_CELADON, 28,  12); return true; }
        // Celadon
        if (currentMapId == MAP_CELADON   && nx >= 30)  { loadMap(MAP_ROUTE7,   1,   7); return true; }
        if (currentMapId == MAP_CELADON   && nx <  0)   { loadMap(MAP_ROUTE16, 38,  12); return true; }
        // Route 8
        if (currentMapId == MAP_ROUTE8    && nx <  0)   { loadMap(MAP_SAFFRON, 23,  12); return true; }
        if (currentMapId == MAP_ROUTE8    && nx >= 30)  { loadMap(MAP_LAVENDER, 1,  10); return true; }
        // Lavender
        if (currentMapId == MAP_LAVENDER  && nx <  0)   { loadMap(MAP_ROUTE8,  28,   7); return true; }
        if (currentMapId == MAP_LAVENDER  && ny >= 20)  { loadMap(MAP_ROUTE12,  9,   1); return true; }
        // Route 12
        if (currentMapId == MAP_ROUTE12   && ny <  0)   { loadMap(MAP_LAVENDER, 9,  18); return true; }
        if (currentMapId == MAP_ROUTE12   && ny >= 40)  { loadMap(MAP_ROUTE13,  9,   1); return true; } // → Route 13
        // Route 13
        if (currentMapId == MAP_ROUTE13   && ny <  0)   { loadMap(MAP_ROUTE12,  9,  38); return true; }
        if (currentMapId == MAP_ROUTE13   && nx <  0)   { loadMap(MAP_ROUTE14,  38,  9); return true; }
        // Route 14
        if (currentMapId == MAP_ROUTE14   && nx >= 40)  { loadMap(MAP_ROUTE13,  1,   9); return true; }
        if (currentMapId == MAP_ROUTE14   && ny >= 20)  { loadMap(MAP_ROUTE15,  9,   1); return true; }
        // Route 15
        if (currentMapId == MAP_ROUTE15   && ny <  0)   { loadMap(MAP_ROUTE14,  9,  18); return true; }
        if (currentMapId == MAP_ROUTE15   && nx <  0)   { loadMap(MAP_FUCHSIA, 28,   9); return true; }
        // Route 16
        if (currentMapId == MAP_ROUTE16   && nx >= 40)  { loadMap(MAP_CELADON,  1,  12); return true; }
        if (currentMapId == MAP_ROUTE16   && nx <  0)   { loadMap(MAP_ROUTE15, 38,   9); return true; }
        // Fuchsia
        if (currentMapId == MAP_FUCHSIA   && nx >= 30)  { loadMap(MAP_ROUTE15,  1,   9); return true; }
        if (currentMapId == MAP_FUCHSIA   && ny >= 25)  { loadMap(MAP_ROUTE19, 14,   1); return true; }
        // Route 19
        if (currentMapId == MAP_ROUTE19   && ny <  0)   { loadMap(MAP_FUCHSIA, 14,  23); return true; }
        if (currentMapId == MAP_ROUTE19   && ny >= 40)  { loadMap(MAP_ROUTE20, 38,   9); return true; }
        // Route 20
        if (currentMapId == MAP_ROUTE20   && nx >= 40)  { loadMap(MAP_ROUTE19, 14,  38); return true; }
        if (currentMapId == MAP_ROUTE20   && nx <  0)   { loadMap(MAP_CINNABAR,18,   9); return true; }
        // Cinnabar
        if (currentMapId == MAP_CINNABAR  && nx >= 20)  { loadMap(MAP_ROUTE20,  1,   9); return true; }
        if (currentMapId == MAP_CINNABAR  && ny <  0)   { loadMap(MAP_ROUTE21,  9,  38); return true; }
        // Route 21
        if (currentMapId == MAP_ROUTE21   && ny >= 40)  { loadMap(MAP_CINNABAR, 9,   1); return true; }
        if (currentMapId == MAP_ROUTE21   && ny <  0)   { loadMap(MAP_PALLET,   9,  13); return true; }
        // Forest
        if (currentMapId == MAP_FOREST    && ny <  0)   { loadMap(MAP_ROUTE2,   8,   2); return true; }
        if (currentMapId == MAP_FOREST    && ny >= 30)  { loadMap(MAP_ROUTE2,   8,  15); return true; }

        return false;
    }

    // -----------------------------------------------------------------------
    // Door / warp tile handler
    // -----------------------------------------------------------------------
    private void handleDoorTile(int tile)
    {
        if (tile == 98) { loadMap(returnMapId, 14, 14); return; }
        if (tile == 99) {
            if      (currentMapId == MAP_GYM_PEWTER)   loadMap(MAP_PEWTER,   9, 10);
            else if (currentMapId == MAP_GYM_CERULEAN) loadMap(MAP_CERULEAN, 11,  9);
            else if (currentMapId == MAP_GYM_VERMILION)loadMap(MAP_VERMILION, 7, 17);
            else if (currentMapId == MAP_GYM_CELADON)  loadMap(MAP_CELADON,  13, 21);
            else if (currentMapId == MAP_GYM_FUCHSIA)  loadMap(MAP_FUCHSIA,   7, 19);
            else if (currentMapId == MAP_GYM_CINNABAR) loadMap(MAP_CINNABAR, 15,  9);
            else if (currentMapId == MAP_MT_MOON)      loadMap(MAP_ROUTE4,    2,  7);
            else if (currentMapId == MAP_WARDEN_HOUSE) loadMap(MAP_FUCHSIA,  23,  9);
            return;
        }
        if (tile == 201) { loadMap(MAP_GYM_PEWTER,   7, 13); return; }
        if (tile == 202) { returnMapId = currentMapId; loadMap(MAP_MART,   5,  8); return; }
        if (tile == 203) { returnMapId = currentMapId; loadMap(MAP_CENTER, 5,  8); return; }
        if (tile == 204) { loadMap(MAP_GYM_CERULEAN,  7, 16); return; }
        if (tile == 205) { loadMap(MAP_FOREST,        10, 28); return; }
        if (tile == 208) { loadMap(MAP_GYM_VERMILION, 7, 13); return; }
        if (tile == 210) { loadMap(MAP_GYM_CELADON,   7, 13); return; }
        if (tile == 212) { loadMap(MAP_GYM_FUCHSIA,   7, 13); return; }
        if (tile == 213) { loadMap(MAP_GYM_CINNABAR,  7, 13); return; }
        if (tile == 214) { loadMap(MAP_WARDEN_HOUSE,  5, 10); return; }
    }

    // -----------------------------------------------------------------------
    // Public helpers
    // -----------------------------------------------------------------------
    public void refreshMap() { loadMapInternal(currentMapId, playerX, playerY, false); }
    public void respawn()    { loadMap(MAP_PALLET, 10, 10); }
    public int  getMapId()   { return currentMapId; }

    /** Called by GameLauncher before switching to the battle screen. */
    public void stopMovement() {
        // Clear all held keys so no direction is queued when we return
        java.util.Arrays.fill(keysHeld, false);
        // Snap player to current tile instantly — no mid-step ghost position
        if (isMoving) {
            playerX  = targetX;
            playerY  = targetY;
            offsetX  = 0f;
            offsetY  = 0f;
            isMoving = false;
        }
        moveTimer.stop();
    }

    /** Called by GameLauncher after returning from the battle screen. */
    public void resumeMovement() {
        java.util.Arrays.fill(keysHeld, false); // clear stale key state
        if (!moveTimer.isRunning()) moveTimer.start();
    }
    public int  getPlayerX() { return playerX; }
    public int  getPlayerY() { return playerY; }
    public int  getMapWidth()  { return currentMap != null ? currentMap.length    : 20; }
    public int  getMapHeight() { return currentMap != null ? currentMap[0].length : 20; }

    private void loadMap(int mapId, int startX, int startY) { loadMapInternal(mapId, startX, startY, true); }

    private void loadMapInternal(int mapId, int startX, int startY, boolean movePlayer)
    {
        currentMapId = mapId;
        if (movePlayer) {
            playerX = startX; playerY = startY;
            offsetX = 0f;     offsetY = 0f;
            isMoving = false;
        }

        if (allMaps[mapId] == null)
        {
            npcs.clear(); labels.clear();

            switch (mapId) {
                case MAP_PALLET:       generatePalletTown();    break;
                case MAP_ROUTE1:       generateRoute1();        break;
                case MAP_VIRIDIAN:     generateViridian();      break;
                case MAP_ROUTE2:       generateRoute2();        break;
                case MAP_FOREST:       generateForest();        break;
                case MAP_PEWTER:       generatePewter();        break;
                case MAP_ROUTE3:       generateRoute3();        break;
                case MAP_MT_MOON:      generateMtMoon();        break;
                case MAP_ROUTE4:       generateRoute4();        break;
                case MAP_CERULEAN:     generateCeruleanCity();  break;
                case MAP_ROUTE24:      generateRoute24();       break;
                case MAP_ROUTE5:       generateRoute5();        break;
                case MAP_SAFFRON:      generateSaffronCity();   break;
                case MAP_ROUTE6:       generateRoute6();        break;
                case MAP_VERMILION:    generateVermilionCity(); break;
                case MAP_ROUTE11:      generateRoute11();       break;
                case MAP_ROUTE9:       generateRoute9();        break;
                case MAP_ROUTE7:       generateRoute7();        break;
                case MAP_CELADON:      generateCeladonCity();   break;
                case MAP_ROUTE8:       generateRoute8();        break;
                case MAP_LAVENDER:     generateLavenderTown();  break;
                case MAP_ROUTE12:      generateRoute12();       break;
                case MAP_ROUTE13:      generateRoute13();       break;
                case MAP_ROUTE14:      generateRoute14();       break;
                case MAP_ROUTE16:      generateRoute16();       break;
                case MAP_ROUTE15:      generateRoute15();       break;
                case MAP_FUCHSIA:      generateFuchsiaCity();   break;
                case MAP_ROUTE19:      generateRoute19();       break;
                case MAP_ROUTE20:      generateRoute20();       break;
                case MAP_CINNABAR:     generateCinnabarIsland();break;
                case MAP_ROUTE21:      generateRoute21();       break;
                case MAP_GYM_PEWTER:   generateGymPewter();    break;
                case MAP_GYM_CERULEAN: generateGymCerulean();  break;
                case MAP_GYM_VERMILION:generateGymVermilion(); break;
                case MAP_GYM_CELADON:  generateGymCeladon();   break;
                case MAP_GYM_FUCHSIA:  generateGymFuchsia();   break;
                case MAP_GYM_CINNABAR: generateGymCinnabar();  break;
                case MAP_CENTER:       generateCenter();        break;
                case MAP_MART:         generateMart();          break;
                case MAP_WARDEN_HOUSE: generateWardenHouse();  break;
            }

            allMaps[mapId]   = currentMap;
            allNPCs.set(mapId,   new ArrayList<>(npcs));
            allLabels.set(mapId, new ArrayList<>(labels));
        } else {
            currentMap = allMaps[mapId];
            npcs       = allNPCs.get(mapId);
            labels     = allLabels.get(mapId);
        }
        repaint();
    }

    // -----------------------------------------------------------------------
    // MAP GENERATION — Pokémon Red route order
    // -----------------------------------------------------------------------
    private void generatePalletTown() {
        currentMap = new int[20][15]; fillMap(0); drawBorder(20, 15);
        currentMap[9][0] = 0; currentMap[10][0] = 0;
        currentMap[9][14] = 5; currentMap[10][14] = 5;
        buildHouse(12, 8, 5, 4, 200, "OAK'S LAB");
        npcs.add(new NPC("Oak", 14, 13, "OAK", "NONE"));
        labels.add(new MapLabel("PALLET TOWN", 2, 2));
    }

    private void generateRoute1() {
        currentMap = new int[14][30]; fillMap(0); drawBorder(14, 30);
        for (int y = 0; y < 30; y++) { currentMap[6][y] = 3; currentMap[7][y] = 3; }
        for (int y = 5; y < 25; y++) { currentMap[3][y] = 4; currentMap[4][y] = 4; currentMap[9][y] = 4; currentMap[10][y] = 4; }
        labels.add(new MapLabel("ROUTE 1", 2, 2));
    }

    private void generateViridian() {
        currentMap = new int[20][20]; fillMap(3); drawBorder(20, 20);
        currentMap[9][19] = 3; currentMap[10][19] = 3; currentMap[9][0] = 3; currentMap[10][0] = 3;
        buildHouse(14,  4, 4, 4, 202, "MART");
        buildHouse(14, 12, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("VIRIDIAN CITY", 2, 2));
    }

    private void generateRoute2() {
        currentMap = new int[16][30]; fillMap(0); drawBorder(16, 30);
        for (int y = 0; y < 30; y++) { currentMap[7][y] = 3; currentMap[8][y] = 3; }
        buildHouse(5, 12, 6, 4, 205, "VIRIDIAN FOREST");
        labels.add(new MapLabel("ROUTE 2", 2, 2));
    }

    private void generateForest() {
        currentMap = new int[20][30]; fillMap(0); drawBorder(20, 30);
        java.util.Random r = new java.util.Random(42);
        for (int x = 1; x < 19; x++)
            for (int y = 1; y < 28; y++)
                if (x != 9 && x != 10 && r.nextInt(5) == 0) currentMap[x][y] = 1;
        for (int y = 0; y < 30; y++) { currentMap[9][y] = 3; currentMap[10][y] = 3; }
        for (int x = 2; x < 6; x++) for (int y = 5; y < 15; y++) currentMap[x][y] = 4;
        currentMap[9][29] = 3; currentMap[10][29] = 3; currentMap[9][0] = 3; currentMap[10][0] = 3;
        NPC bug = new NPC("Bug Catcher", 8, 20, "TRAINER", "BUG1");
        bug.party = new Pokemon("Caterpie", "BUG", 6);
        npcs.add(bug);
        labels.add(new MapLabel("VIRIDIAN FOREST", 2, 2));
    }

    private void generatePewter() {
        currentMap = new int[20][20]; fillMap(3); drawBorder(20, 20);
        currentMap[9][19] = 3; currentMap[10][19] = 3; currentMap[19][9] = 3; currentMap[19][10] = 3;
        buildHouse(4,  4, 6, 5, 201, "BROCK'S GYM");
        buildHouse(14, 12, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("PEWTER CITY", 2, 2));
    }

    private void generateRoute3() {
        currentMap = new int[40][15]; fillMap(0); drawBorder(40, 15);
        for (int x = 0; x < 40; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 5; x < 12; x++) for (int y = 3; y < 6; y++) currentMap[x][y] = 4;
        NPC youngster = new NPC("Youngster", 15, 6, "TRAINER", "R3_TR1");
        youngster.party = new Pokemon("Rattata", "NORMAL", 11);
        npcs.add(youngster);
        labels.add(new MapLabel("ROUTE 3", 2, 5));
    }

    private void generateMtMoon() {
        currentMap = new int[30][30]; fillMap(3); drawBorder(30, 30);
        java.util.Random sr = new java.util.Random(12345);
        for (int i = 0; i < 40; i++) currentMap[sr.nextInt(26) + 2][sr.nextInt(26) + 2] = 1;
        NPC rocket = new NPC("Rocket Grunt", 15, 15, "TRAINER", "MTM_R1");
        rocket.party = new Pokemon("Zubat", "POISON", 12);
        npcs.add(rocket);
        currentMap[29][14] = 99;
        labels.add(new MapLabel("MT. MOON", 2, 2));
        labels.add(new MapLabel("EXIT >", 25, 15));
    }

    private void generateRoute4() {
        currentMap = new int[30][15]; fillMap(0); drawBorder(30, 15);
        for (int x = 0; x < 30; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 5; x < 20; x++) currentMap[x][5] = 1;
        labels.add(new MapLabel("ROUTE 4", 15, 2));
    }

    private void generateCeruleanCity() {
        currentMap = new int[22][20]; fillMap(3); drawBorder(22, 20);
        currentMap[0][9]  = 3; currentMap[0][10]  = 3;  // West exit (Route 4)
        currentMap[10][0] = 3; currentMap[11][0]  = 3;  // North exit (Route 24)
        currentMap[10][19]= 3; currentMap[11][19] = 3;  // South exit (Route 5)
        buildHouse(8,  5, 6, 5, 204, "MISTY'S GYM");
        buildHouse(16, 12, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("CERULEAN CITY", 2, 2));
    }

    // Route 24 — north of Cerulean (Nugget Bridge area)
    private void generateRoute24() {
        currentMap = new int[16][20]; fillMap(0); drawBorder(16, 20);
        for (int y = 0; y < 20; y++) { currentMap[7][y] = 3; currentMap[8][y] = 3; }
        for (int x = 3; x < 7; x++) for (int y = 5; y < 15; y++) currentMap[x][y] = 4;
        NPC camper = new NPC("Camper", 8, 10, "TRAINER", "R24_TR1");
        camper.party = new Pokemon("Oddish", "GRASS", 14);
        npcs.add(camper);
        labels.add(new MapLabel("ROUTE 24", 2, 2));
    }

    private void generateRoute5() {
        currentMap = new int[20][30]; fillMap(0); drawBorder(20, 30);
        for (int y = 0; y < 30; y++) { currentMap[9][y] = 3; currentMap[10][y] = 3; }
        for (int x = 4; x < 9; x++) for (int y = 5; y < 15; y++) currentMap[x][y] = 4;
        buildHouse(15, 12, 4, 4, 206, "DAYCARE");
        labels.add(new MapLabel("ROUTE 5", 2, 2));
    }

    private void generateSaffronCity() {
        currentMap = new int[25][25]; fillMap(3); drawBorder(25, 25);
        currentMap[10][0]  = 3; currentMap[11][0]  = 3;
        currentMap[10][24] = 3; currentMap[11][24] = 3;
        currentMap[0][12]  = 3; currentMap[0][13]  = 3;
        currentMap[24][12] = 3; currentMap[24][13] = 3;
        buildHouse(4,  4, 8, 6, 207, "SILPH CO.");
        buildHouse(16, 16, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("SAFFRON CITY", 2, 2));
    }

    private void generateRoute6() {
        currentMap = new int[20][20]; fillMap(0); drawBorder(20, 20);
        for (int y = 0; y < 20; y++) { currentMap[9][y] = 3; currentMap[10][y] = 3; }
        for (int x = 4; x < 8; x++) for (int y = 5; y < 15; y++) currentMap[x][y] = 4;
        labels.add(new MapLabel("ROUTE 6", 2, 2));
    }

    private void generateVermilionCity() {
        currentMap = new int[25][25]; fillMap(3); drawBorder(25, 25);
        currentMap[9][0]   = 3; currentMap[10][0]  = 3;
        currentMap[24][10] = 3; currentMap[24][11] = 3;
        for (int x = 1; x < 24; x++) for (int y = 20; y < 24; y++) currentMap[x][y] = 5;
        for (int y = 20; y < 24; y++) { currentMap[11][y] = 3; currentMap[12][y] = 3; }
        buildHouse(4,  12, 6, 5, 208, "SURGE'S GYM");
        buildHouse(16,  4, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("VERMILION CITY", 2, 2));
    }

    // Route 11 — Vermilion east to Route 9
    private void generateRoute11() {
        currentMap = new int[40][15]; fillMap(0); drawBorder(40, 15);
        for (int x = 0; x < 40; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 10; x < 30; x++) for (int y = 3; y < 6; y++) currentMap[x][y] = 4;
        NPC sailor = new NPC("Sailor", 20, 8, "TRAINER", "R11_TR1");
        sailor.party = new Pokemon("Machop", "FIGHTING", 16);
        npcs.add(sailor);
        labels.add(new MapLabel("ROUTE 11", 5, 2));
    }

    // Route 9 — connects Route 11 → Lavender (replaces separate Rock Tunnel)
    private void generateRoute9() {
        currentMap = new int[40][15]; fillMap(0); drawBorder(40, 15);
        for (int x = 0; x < 40; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 8; x < 32; x++) for (int y = 3; y < 6; y++) currentMap[x][y] = 4;
        NPC hiker = new NPC("Hiker", 20, 8, "TRAINER", "R9_TR1");
        hiker.party = new Pokemon("Geodude", "ROCK", 18);
        npcs.add(hiker);
        labels.add(new MapLabel("ROUTE 9", 5, 2));
    }

    private void generateRoute7() {
        currentMap = new int[20][15]; fillMap(0); drawBorder(20, 15);
        for (int x = 0; x < 20; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 5; x < 15; x++) for (int y = 3; y < 6; y++) currentMap[x][y] = 4;
        labels.add(new MapLabel("ROUTE 7", 5, 2));
    }

    private void generateCeladonCity() {
        currentMap = new int[30][25]; fillMap(3); drawBorder(30, 25);
        currentMap[0][12]  = 3; currentMap[0][13]  = 3;
        currentMap[29][12] = 3; currentMap[29][13] = 3;
        buildHouse(4,  4, 10, 8, 209, "DEPT. STORE");
        buildHouse(10, 16, 6, 5, 210, "ERIKA'S GYM");
        buildHouse(20, 12, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("CELADON CITY", 2, 2));
    }

    private void generateRoute8() {
        currentMap = new int[30][15]; fillMap(0); drawBorder(30, 15);
        for (int x = 0; x < 30; x++) { currentMap[x][7] = 3; currentMap[x][8] = 3; }
        for (int x = 5; x < 25; x++) for (int y = 9; y < 12; y++) currentMap[x][y] = 4;
        NPC biker = new NPC("Biker", 15, 8, "TRAINER", "R8_TR1");
        biker.party = new Pokemon("Koffing", "POISON", 20);
        npcs.add(biker);
        labels.add(new MapLabel("ROUTE 8", 10, 2));
    }

    private void generateLavenderTown() {
        currentMap = new int[20][20]; fillMap(3); drawBorder(20, 20);
        currentMap[0][10]  = 3; currentMap[0][11]  = 3;
        currentMap[9][19]  = 3; currentMap[10][19] = 3;
        buildHouse(12,  4, 6, 8, 211, "POKÉMON TOWER");
        buildHouse(4,  10, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("LAVENDER TOWN", 2, 2));
    }

    // Route 12 → Route 13 (south of Lavender)
    private void generateRoute12() {
        currentMap = new int[20][40]; fillMap(0); drawBorder(20, 40);
        for (int y = 0; y < 40; y++) { currentMap[9][y] = 3; currentMap[10][y] = 3; }
        for (int x = 13; x < 19; x++) for (int y = 5; y < 35; y++) currentMap[x][y] = 5;
        for (int x = 4;  x < 8;  x++) for (int y = 10; y < 30; y++) currentMap[x][y] = 4;
        NPC fisher = new NPC("Fisherman", 8, 20, "TRAINER", "R12_TR1");
        fisher.party = new Pokemon("Magikarp", "WATER", 15);
        npcs.add(fisher);
        labels.add(new MapLabel("ROUTE 12", 2, 5));
    }

    // Route 13 — diagonal connector (simplified as horizontal)
    private void generateRoute13() {
        currentMap = new int[40][20]; fillMap(0); drawBorder(40, 20);
        for (int x = 0; x < 40; x++) { currentMap[x][9] = 3; currentMap[x][10] = 3; }
        for (int x = 5; x < 35; x++) for (int y = 13; y < 17; y++) currentMap[x][y] = 4;
        NPC birdKeeper = new NPC("Bird Keeper", 20, 10, "TRAINER", "R13_TR1");
        birdKeeper.party = new Pokemon("Fearow", "FLYING", 22);
        npcs.add(birdKeeper);
        labels.add(new MapLabel("ROUTE 13", 15, 2));
    }

    // Route 14 — connects Route 13 → Route 15 (vertical)
    private void generateRoute14() {
        currentMap = new int[40][20]; fillMap(0); drawBorder(40, 20);
        for (int x = 0; x < 40; x++) { currentMap[x][9] = 3; currentMap[x][10] = 3; }
        for (int x = 5; x < 30; x++) for (int y = 3; y < 8; y++) currentMap[x][y] = 4;
        labels.add(new MapLabel("ROUTE 14", 15, 2));
    }

    private void generateRoute15() {
        currentMap = new int[40][20]; fillMap(0); drawBorder(40, 20);
        for (int x = 0; x < 40; x++) { currentMap[x][9] = 3; currentMap[x][10] = 3; }
        for (int x = 5; x < 35; x++) for (int y = 13; y < 17; y++) currentMap[x][y] = 4;
        labels.add(new MapLabel("ROUTE 15", 15, 2));
    }

    private void generateRoute16() {
        currentMap = new int[40][20]; fillMap(0); drawBorder(40, 20);
        for (int x = 0; x < 40; x++) { currentMap[x][12] = 3; currentMap[x][13] = 3; }
        for (int x = 10; x < 30; x++) for (int y = 4; y < 10; y++) currentMap[x][y] = 4;
        NPC roughneck = new NPC("Roughneck", 20, 14, "TRAINER", "R16_TR1");
        roughneck.party = new Pokemon("Grimer", "POISON", 22);
        npcs.add(roughneck);
        labels.add(new MapLabel("ROUTE 16", 15, 2));
    }

    private void generateFuchsiaCity() {
        currentMap = new int[30][25]; fillMap(3); drawBorder(30, 25);
        currentMap[29][9]  = 3; currentMap[29][10] = 3;
        currentMap[14][24] = 3; currentMap[15][24] = 3;
        buildHouse(4,  14, 6, 5, 212, "KOGA'S GYM");
        buildHouse(20,  4, 6, 5, 214, "WARDEN'S HOUSE");
        buildHouse(14,  4, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("FUCHSIA CITY", 2, 2));
    }

    private void generateRoute19() {
        currentMap = new int[20][40]; fillMap(5); drawBorder(20, 40);
        labels.add(new MapLabel("ROUTE 19 (SEA)", 2, 5));
    }

    private void generateRoute20() {
        currentMap = new int[40][20]; fillMap(5); drawBorder(40, 20);
        labels.add(new MapLabel("ROUTE 20 (SEA)", 15, 2));
    }

    private void generateCinnabarIsland() {
        currentMap = new int[20][20]; fillMap(0); drawBorder(20, 20);
        currentMap[9][0]   = 5; currentMap[10][0]  = 5;
        currentMap[19][9]  = 5; currentMap[19][10] = 5;
        buildHouse(12,  4, 6, 5, 213, "BLAINE'S GYM");
        buildHouse(4,  12, 6, 5, 215, "POKÉMON LAB");
        buildHouse(4,   4, 4, 4, 203, "POKECENTER");
        labels.add(new MapLabel("CINNABAR ISLAND", 2, 2));
    }

    private void generateRoute21() {
        currentMap = new int[20][40]; fillMap(5); drawBorder(20, 40);
        labels.add(new MapLabel("ROUTE 21 (SEA)", 2, 5));
    }

    // -----------------------------------------------------------------------
    // Gyms & Interiors
    // -----------------------------------------------------------------------
    private void generateGymPewter() {
        currentMap = new int[15][15]; fillMap(3); drawBorder(15, 15); currentMap[7][14] = 99;
        NPC brock = new NPC("Brock", 7, 2, "LEADER", "LEADER");
        brock.party = new Pokemon("Onix", "ROCK", 14); npcs.add(brock);
    }
    private void generateGymCerulean() {
        currentMap = new int[15][18]; fillMap(3); drawBorder(15, 18);
        for (int x = 2; x <= 12; x++) for (int y = 4; y <= 12; y++) currentMap[x][y] = 5;
        for (int y = 4; y <= 12; y++) currentMap[7][y] = 3;
        currentMap[7][17] = 99;
        NPC misty = new NPC("Misty", 7, 3, "LEADER", "LEADER_MISTY");
        misty.party = new Pokemon("Starmie", "WATER", 21); npcs.add(misty);
    }
    private void generateGymVermilion() {
        currentMap = new int[15][15]; fillMap(3); drawBorder(15, 15); currentMap[7][14] = 99;
        NPC surge = new NPC("Lt. Surge", 7, 2, "LEADER", "LEADER_SURGE");
        surge.party = new Pokemon("Raichu", "ELECTRIC", 24); npcs.add(surge);
    }
    private void generateGymCeladon() {
        currentMap = new int[15][15]; fillMap(3); drawBorder(15, 15); currentMap[7][14] = 99;
        for (int x = 2; x <= 12; x++) for (int y = 4; y <= 12; y++) if (x != 7) currentMap[x][y] = 1;
        NPC erika = new NPC("Erika", 7, 2, "LEADER", "LEADER_ERIKA");
        erika.party = new Pokemon("Vileplume", "GRASS", 29); npcs.add(erika);
    }
    private void generateGymFuchsia() {
        currentMap = new int[15][15]; fillMap(3); drawBorder(15, 15); currentMap[7][14] = 99;
        for (int x = 2; x <= 12; x += 2) for (int y = 4; y <= 10; y++) currentMap[x][y] = 1;
        NPC koga = new NPC("Koga", 7, 2, "LEADER", "LEADER_KOGA");
        koga.party = new Pokemon("Weezing", "POISON", 37); npcs.add(koga);
    }
    private void generateGymCinnabar() {
        currentMap = new int[15][15]; fillMap(3); drawBorder(15, 15); currentMap[7][14] = 99;
        NPC blaine = new NPC("Blaine", 7, 2, "LEADER", "LEADER_BLAINE");
        blaine.party = new Pokemon("Arcanine", "FIRE", 47); npcs.add(blaine);
    }
    private void generateCenter() {
        currentMap = new int[12][10]; fillMap(3); drawBorder(12, 10);
        for (int x = 4; x <= 7; x++) currentMap[x][3] = 2;
        currentMap[5][9] = 98; currentMap[6][9] = 98;
        npcs.add(new NPC("Nurse Joy", 5, 2, "HEALER", "JOY"));
    }
    private void generateMart() {
        currentMap = new int[12][10]; fillMap(3); drawBorder(12, 10);
        currentMap[5][9] = 98; currentMap[6][9] = 98;
        npcs.add(new NPC("Clerk", 5, 3, "MART", "SHOP"));
    }
    private void generateWardenHouse() {
        currentMap = new int[12][12]; fillMap(3); drawBorder(12, 12);
        currentMap[5][11] = 99; currentMap[6][11] = 99;
        npcs.add(new NPC("Warden", 5, 4, "WARDEN", "NONE"));
    }

    // -----------------------------------------------------------------------
    // Map utilities
    // -----------------------------------------------------------------------
    private void fillMap(int val) {
        for (int x = 0; x < currentMap.length; x++)
            for (int y = 0; y < currentMap[0].length; y++)
                currentMap[x][y] = val;
    }
    private void drawBorder(int w, int h) {
        for (int x = 0; x < w; x++) { currentMap[x][0] = 1; currentMap[x][h - 1] = 1; }
        for (int y = 0; y < h; y++) { currentMap[0][y] = 1; currentMap[w - 1][y] = 1; }
    }
    private void buildHouse(int x, int y, int w, int h, int doorID, String labelText) {
        for (int i = x; i < x + w; i++) for (int j = y; j < y + h; j++) currentMap[i][j] = 2;
        currentMap[x + (w / 2)][y + h - 1] = doorID;
        labels.add(new MapLabel(labelText, x + (w / 2), y - 1));
    }

    // -----------------------------------------------------------------------
    // Interact (Space bar)
    // -----------------------------------------------------------------------
    private void interact() {
        for (NPC n : npcs) {
            if (Math.abs(n.x - playerX) <= 1 && Math.abs(n.y - playerY) <= 1) {
                if (n.type.equals("OAK")) {
                    if (!GameLauncher.hasStarter) {
                        String[] starters = {"Charmander", "Squirtle", "Bulbasaur"};
                        int c = JOptionPane.showOptionDialog(this, "Choose your starter!", "Oak",
                                0, 3, null, starters, 0);
                        if (c == 0) GameLauncher.party.add(new Pokemon("Charmander", "FIRE",  5));
                        if (c == 1) GameLauncher.party.add(new Pokemon("Squirtle",  "WATER", 5));
                        if (c == 2) GameLauncher.party.add(new Pokemon("Bulbasaur", "GRASS", 5));
                        if (c != -1) GameLauncher.hasStarter = true;
                    }
                } else if (n.type.equals("WARDEN")) {
                    if (!canSurf) {
                        JOptionPane.showMessageDialog(this,
                            "Safari Warden: Thanks for finding my Gold Teeth!\n\nTake this HM! (You can now SURF!)");
                        canSurf = true;
                    } else {
                        JOptionPane.showMessageDialog(this, "Safari Warden: Enjoy surfing around Kanto!");
                    }
                } else if (n.type.equals("MART")) {
                    String[] items = {"Potion - $300", "Poké Ball - $200", "Cancel"};
                    int choice = JOptionPane.showOptionDialog(this,
                        "Welcome! What would you like?", "Poké Mart",
                        0, 3, null, items, items[0]);
                    if (choice == 0 && GameLauncher.money >= 300) {
                        GameLauncher.money -= 300;
                        GameLauncher.bag.add("Potion");
                        JOptionPane.showMessageDialog(this, "Bought a Potion!");
                    } else if (choice == 1 && GameLauncher.money >= 200) {
                        GameLauncher.money -= 200;
                        GameLauncher.bag.add("Poké Ball");
                        JOptionPane.showMessageDialog(this, "Bought a Poké Ball!");
                    }
                }
                return;
            }
        }
    }

    // -----------------------------------------------------------------------
    // Rendering — camera follows player with sub-tile offset for smooth scroll
    // -----------------------------------------------------------------------
    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);

        // Camera origin: centre screen on player's pixel position
        float playerPixelX = playerX * TILE_SIZE + offsetX;
        float playerPixelY = playerY * TILE_SIZE + offsetY;
        int camX = (int)(800 / 2 - playerPixelX);
        int camY = (int)(600 / 2 - playerPixelY);

        // Draw tiles
        for (int x = 0; x < currentMap.length; x++) {
            for (int y = 0; y < currentMap[0].length; y++) {
                int id = currentMap[x][y];
                Color c;
                switch (id) {
                    case 0:  c = new Color(50, 205, 50);   break; // Grass
                    case 4:  c = new Color(0,  100,  0);   break; // Tall Grass
                    case 3:  c = Color.LIGHT_GRAY;          break; // Path
                    case 1:  c = Color.GRAY;                break; // Wall
                    case 2:  c = new Color(178, 34, 34);   break; // Building
                    case 5:  c = new Color(30, 144, 255);  break; // Water
                    default: c = new Color(200, 180, 140); break; // Door/misc
                }
                g.setColor(c);
                g.fillRect(x * TILE_SIZE + camX, y * TILE_SIZE + camY, TILE_SIZE, TILE_SIZE);

                // Thin grid lines for readability
                g.setColor(new Color(0, 0, 0, 30));
                g.drawRect(x * TILE_SIZE + camX, y * TILE_SIZE + camY, TILE_SIZE, TILE_SIZE);
            }
        }

        // Draw NPCs
        for (NPC n : npcs) {
            Color nc;
            switch (n.type) {
                case "HEALER":  nc = Color.PINK;    break;
                case "TRAINER": nc = Color.CYAN;    break;
                case "LEADER":  nc = Color.MAGENTA; break;
                case "WARDEN":  nc = Color.YELLOW;  break;
                default:        nc = Color.ORANGE;  break;
            }
            g.setColor(nc);
            g.fillOval(n.x * TILE_SIZE + camX + 5, n.y * TILE_SIZE + camY + 5, TILE_SIZE - 10, TILE_SIZE - 10);
        }

        // Draw Player — pixel-perfect position using offsetX/Y
        float px = playerX * TILE_SIZE + offsetX;
        float py = playerY * TILE_SIZE + offsetY;
        boolean onWater = playerX >= 0 && playerY >= 0
            && playerX < currentMap.length && playerY < currentMap[0].length
            && currentMap[playerX][playerY] == 5;
        g.setColor(onWater ? new Color(135, 206, 250) : Color.RED);
        g.fillOval((int)(px + camX) + 5, (int)(py + camY) + 5, TILE_SIZE - 10, TILE_SIZE - 10);

        // Labels
        g.setColor(Color.WHITE);
        g.setFont(new Font("Arial", Font.BOLD, 14));
        for (MapLabel l : labels)
            g.drawString(l.text, l.x * TILE_SIZE + camX, l.y * TILE_SIZE + camY);

        // Pokédex overlay
        if (showPokedex) {
            g.setColor(new Color(0, 0, 0, 180)); g.fillRect(0, 0, 800, 600);
            g.setColor(new Color(220, 20, 60));  g.fillRoundRect(100, 50, 600, 500, 20, 20);
            g.setColor(Color.WHITE);             g.drawRoundRect(100, 50, 600, 500, 20, 20);
            g.setColor(new Color(30, 30, 30));   g.fillRect(150, 150, 500, 300);
            g.setColor(Color.WHITE);
            g.setFont(new Font("Monospaced", Font.BOLD, 36));
            g.drawString("POKÉDEX", 320, 110);
            g.setFont(new Font("Monospaced", Font.PLAIN, 20));
            g.setColor(Color.GREEN);
            g.drawString("001. Bulbasaur  [SEEN]",  170, 190);
            g.drawString("004. Charmander [OWNED]", 170, 220);
            g.setColor(Color.GRAY);
            g.drawString("007. Squirtle   [???]",   170, 250);
            g.drawString("025. Pikachu    [???]",   170, 280);
            g.setColor(Color.WHITE);
            g.setFont(new Font("Arial", Font.ITALIC, 16));
            g.drawString("Press 'P' to Close", 330, 520);
        }
    }

    // -----------------------------------------------------------------------
    // Inner classes
    // -----------------------------------------------------------------------
    class NPC {
        String name, type, id; int x, y; Pokemon party;
        NPC(String n, int x, int y, String t, String i) { name=n; this.x=x; this.y=y; type=t; id=i; }
    }
    class MapLabel {
        String text; int x, y;
        MapLabel(String t, int x, int y) { text=t; this.x=x; this.y=y; }
    }
}